...
"$WT_BIN" --help | grep -q '^  create' && echo "CLI surface intact"
```
...
`CROSS_DEVICE_COPY_DEGRADATION` (tmpfs forces byte copies — hydration_method
is filesystem-dependent: `byte_copy` on tmpfs, hardlink/CoW elsewhere,
clonefile-based CoW on APFS).
...
```sh
wt --json create demo --dir "$WT_FIXTURE/demo"    # data.files_hydrated > 0
wt --json remove demo --dir "$WT_FIXTURE/demo"    # data.references_released > 0
wt --json sweep --age 0s                          # GC; --age 0s defeats the 15m grace
wt --json scrub --dry-run                         # corrupt a blob first (see features/scrub.md)
wt --json scratch --run 'ls heavy/pkg00'          # runs a command, cleans up after itself
```
...
proves nothing about isolation and pollutes the developer's state.
