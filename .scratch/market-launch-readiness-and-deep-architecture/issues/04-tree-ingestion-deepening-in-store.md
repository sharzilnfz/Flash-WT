...
3. Refactor `HydrationEngine` in `flashwt-cli` (`crates/flashwt-cli/src/hydrate.rs`) to call `DiskStore::ingest_tree`, simplifying `hydrate.rs` by several hundred lines.
...
- [ ] Directory tree ingestion logic lives in `crates/flashwt-store/src/ingest.rs`.
