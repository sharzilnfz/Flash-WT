...
1. Release archive naming in `.github/workflows/release.yml` does not match the archive naming expected by `Formula/wt.rb`, `install.sh`, and `scripts/gen-formula.sh`.
...
3. `install.sh` does not normalize version tags, causing 404 download errors if `WT_VERSION` is specified without or with a leading `v`.
4. Homebrew formula generation and release scripts do not package shell completions.

## Work

1. Standardize release archive naming in `.github/workflows/release.yml` to `wt-v${VERSION}-${{ matrix.target }}.tar.gz`.
2. Add `aarch64-unknown-linux-gnu` target matrix across GitHub Actions release workflow, `Formula/wt.rb`, and `install.sh` target resolution.
...
4. Update `Formula/wt.rb` and `scripts/gen-formula.sh` to package and install shell completions for Homebrew users.
...
- [ ] `Formula/wt.rb` and `scripts/gen-formula.sh` configure shell completion installation.
