...
shape for `flashwt create` and builds the founding end-to-end test rig: tests run
...
- [x] `flashwt create --help` works and argument shape is documented in the ticket thread
...
```
flashwt create <NAME> [--manifest <PATH>] [--dir <PATH>]
```

- `<NAME>` — required positional. Used as the git branch name.
- `--manifest <PATH>` — optional. Manifest listing heavy directories,
  gitignore syntax. Defaults to `.flashwtinclude` at the repository root.
- `--dir <PATH>` — optional destination for the new worktree. Defaults
  to a sibling of the current repository named `<repo>-<NAME>` (for
  example, repo `origin` + `flashwt create demo` produces `../origin-demo`).
- Exit code 0 on success, 1 with a message on `flashwt:` stderr on failure
...
- `crates/flashwt-copy` — the copy-backend trait (`CopyBackend`,
  `BackendKind`, `Safety`, `Error`). Ticket 03 implements backends
  here. `BackendKind::Hardlink` reports `Safety::UnsafePending` until
  ticket 07.
- `crates/flashwt-store` — the store trait (`Store`, `ContentId`, `Error`).
  Ticket 04 implements the real store here.
- `crates/flashwt-cli` — the `flashwt` binary. Depends on both crates. Owns all
...
Test rig lives in `crates/flashwt-cli/tests/`: `common/mod.rs` builds a
...
via `CARGO_BIN_EXE_flashwt`. Four tests pass: help shape, smoke `flashwt create
