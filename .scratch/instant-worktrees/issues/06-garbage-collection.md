...
- [x] `flashwt remove` or equivalent releases worktree references in the store
...
`crates/flashwt-cli/src/gc.rs`: `flashwt remove NAME` resolves the worktree's
linked git dir while it still exists, reads `flashwt-hydrated.tsv`, releases
...
the ledger with it. `flashwt sweep --age <dur>` (default 7d; `0s`, `90s`,
`10m`, `24h`, `7d` accepted) reclaims entries whose ref count is zero
and whose object mtime predates the cutoff.

`crates/flashwt-store`: `DiskStore::{ids, delete, sweep}` plus
...
Tests live in `crates/flashwt-cli/tests/gc.rs`, asserted only through the
