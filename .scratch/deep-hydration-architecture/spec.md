...
As `wt` has expanded with whole-directory snapshots, mark-and-sweep garbage collection, ephemeral scratch leases, and platform copy acceleration, the CLI orchestrators have become shallow and wide. High-level commands (`wt create`, `wt scratch`, `wt isolate`, `wt sweep`) are forced to manually coordinate low-level lifecycle steps: ingesting files, probing filesystem strategies, managing thread pools, verifying blob checksums, generating sidecar ledgers, recording mirrors, and polling process trees.
...
3. **Encapsulated `SnapshotProjectionEngine`**: Relocate snapshot selection heuristics, v2 delta rebuild calculations, and self-healing blob retries into `wt-store`.
4. **Encapsulated `Materializer`**: Consolidate OS copy primitives (APFS clonefile, Linux reflink, copy_file_range, byte fallback) and permission mode normalization within `wt-copy`.

## User Stories

1. As a developer running `wt create`, I want worktree creation to be atomic and reliable, so that interrupted commands never leave half-initialized mirrors or corrupt cache entries behind.
2. As an AI agent creating ephemeral worktrees via `wt scratch` or `wt isolate`, I want hydration and lease tracking to execute with zero boilerplate, so that sandbox provisioning is fast and durable.
3. As a developer using `wt`, I want heavy directory hydration (`node_modules`, build targets) to automatically select the fastest materialization backend (APFS clonefile, reflink, or copy_file_range) without manual configuration flags.
...
6. As an operator running `wt sweep`, I want dead ephemeral leases and unreferenced objects to be swept deterministically under configured grace periods and storage budgets without spurious warnings.
...
8. As a contributor extending `wt-store`, I want snapshot cache selection and v2 delta rebuild heuristics to be internal storage algorithms, so that the CLI doesn't need to know storage-specific cache rings.
...
11. As a system administrator, I want storage reclamation to respect configured grace periods (`WT_GC_GRACE`), so that concurrent creates and sweeps never race or delete active blobs.
...
- A single `HydrationEngine` module in `wt-cli` / `wt-store` acts as the primary seam for all tree materialization.
...
### Decision 3: Relocation of Snapshot Projection to `wt-store`
- `SnapshotProjectionEngine` moves entirely to `wt-store`.
...
### Decision 4: Self-Contained `Materializer` in `wt-copy`
- The `Materializer` module in `wt-copy` encapsulates filesystem capability detection, backend priority resolution (clonefile -> reflink -> copy_file_range -> buffered copy), and permission mode normalization.
- Mode repair logic (substituting private copies for hardlinked blobs whose exec bits differ from source metadata) is completely internal to `wt-copy`.
...
- **End-to-End Invariant Verification**: Hydration tests will assert that a materialized worktree has exact file byte parity, accurate permission modes, a valid `wt-hydrated.tsv` sidecar, and an atomic mirror in the store.
...
- **Prior Art**: Existing test suites in `crates/wt-cli/tests/` (`cow_materialization.rs`, `gc_mirror.rs`, `snapshots.rs`, `lease_sweep.rs`) will be ported to use the deepened interfaces directly.
