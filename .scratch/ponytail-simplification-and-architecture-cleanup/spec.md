...
Specifically:
1. **Compilation & Link-Time Friction**: The workspace compiles 26 discrete integration test executables in `wt-cli`, 5 in `wt-store`, and 2 in `wt-copy`. Each binary independently links large dependencies (`clap`, `serde`, `libc`), causing CI test times to exceed 5 minutes and bloating debug artifact caches.
...
3. **Speculative Abstractions**: Layers like `Safety::UnsafePending` in `wt-copy`, `HydrationFilter` struct wrappers in `wt-cli`, and single-implementation `Store` / `WorkspaceCleaner` traits in `wt-store` add indirection without providing multi-backend flexibility.
...
Execute a structured, zero-breaking-change ponytail cleanup across `crates/wt-store`, `crates/wt-cli`, `crates/wt-copy`, integration tests, benchmarks, and distribution scripts:
...
4. **Consolidate Integration Test Binaries**: Merge 26 discrete test files in `wt-cli/tests/` into 5 cohesive module targets with a unified `common::Fixture` harness, reducing Cargo link overhead and cutting CI runtime by over 80%.
...
1. As a CLI user, I want `wt-hydrate` to have zero dead code and no unnecessary crate dependencies like `sha2`, so that binary footprint and compile times are minimized.
...
4. As a developer using `wt clean`, I want empty data receipts constructed via standard `Default` derivations, so that early exit paths are concise and consistent.
...
11. As a developer using `wt-copy`, I want dead batch materialization wrappers and speculative safety gates removed, so that the copy engine has a clear and minimal public API.
...
- Replace scratch ID generation with standard library formatting (`format!("{:08x}", ...)`) or stdlib hasher, dropping the external `sha2` crate dependency from `wt-cli`.
...
- Extract a shared `ingest_entry` closure/helper in `wt-store` to eliminate duplicated validation cache checks, file stat processing, and CAS blob hashing between macOS `getattrlistbulk` walking and portable recursive walking.
...
- Consolidate the 26 discrete test files in `crates/wt-cli/tests/` into 5 cohesive module targets:
...
- Delete obsolete legacy refcount tests (`gc.rs` in `wt-cli/tests` and refcount lock tests in `wt-store/tests/store.rs`) that targeted deleted pre-v0.1 `refs/` plumbing.

### 8. Shared Integration Test Fixture & Assertion Library
- Expand `crates/wt-cli/tests/common/mod.rs` to provide a unified `Fixture` struct with shared repository initialization, Git command execution (`fn git`), and store footprint inspection helpers.
...
- **Modules Tested**:
  - `wt-cli` integration test suite across all subcommands (`new`, `clean`, `list`, `hydrate`, `init`, `scratch`, `scrub`, `demo`).
  - `wt-store` snapshot projection, validation cache, lockfile discovery, and mark-and-sweep GC.
  - `wt-copy` materializer fallback chains, clonefile, reflink, and `copy_file_range`.
- **Prior Art**: Existing integration tests in `crates/wt-cli/tests/` (e.g., `json_output.rs`, `cow_materialization.rs`, `clean.rs`) and test fixtures in `tests/common/mod.rs`.
...
- **Package Management & Build Resolvers**: `wt-hydrate` remains a tool-agnostic filesystem hydration cache; dependency resolution and compiling remain external.

---

## Further Notes

- Expected net codebase reduction: **~3,426 lines of Rust and shell code**, plus removal of **1 external crate dependency** (`sha2` from `wt-cli`).
