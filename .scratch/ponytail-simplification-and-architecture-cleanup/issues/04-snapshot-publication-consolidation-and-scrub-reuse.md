...
2. **Reuse Tree Verification in Scrub**:
   - Refactor `verify_snapshot_dir` in `crates/wt-store/src/scrub.rs` to reuse `DiskStore::find_snapshot` and `paranoid_verify_tree`.
...
3. **Unify Advisory Flock Helper**:
   - Consolidate `RefsLock`, `RefsDirLock`, and `MetadataLock` into a single `FlockGuard` in `crates/wt-store/src/fsutil.rs`.
...
4. **Inline Single-Implementation Traits**:
   - Move methods from single-implementation `Store` trait directly onto `DiskStore` in `crates/wt-store/src/disk.rs`.
...
- Run `cargo test -p wt-store --test scrub` and `cargo test -p wt-store --test snapshots`.
