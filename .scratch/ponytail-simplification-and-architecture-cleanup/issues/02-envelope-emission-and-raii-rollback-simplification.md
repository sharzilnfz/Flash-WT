...
2. **Derive `Default` on Data Envelopes**:
   - Add `#[derive(Default)]` to `CleanData` and `ScratchData` in `crates/wt-cli/src/envelope.rs`.
...
- Run `cargo test -p wt-cli --test json_output`.
