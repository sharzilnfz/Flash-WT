...
/// Ticket 04 mode parity: FICLONE clones only data and OpenOptions'
/// `.mode()` passes through the umask, so the final mode must be set
/// explicitly — a plain `fs::copy` would have preserved it.
#[test]
