...
fn materializer_cross_device_selection_falls_back_to_byte_copy() {
    ...
    let materializer = Materializer::select(StrategyPolicy::Default, true, true, false);
...
fn materializer_unsupported_fs_reports_refusal_reason() {
    let materializer = Materializer::select(StrategyPolicy::Default, false, false, false);
    assert_eq!(materializer.strategy(), "copy-on-write");
    assert_eq!(materializer.selected_backend(), "byte-copy");
    assert!(materializer.refusal_reason().is_some());
    assert!(
