...
use flashwt_store::{Manifest, PublishOptions, SnapshotEntry};
...
fn apfs_defaults_enable_snapshots_without_explicit_env() {
    ...
    let out = fx.flashwt_env(&["create", "snap-auto"], &[]);
    assert!(
        out.status.success(),
...
fn apfs_opt_out_disables_snapshots_via_env() {
    let fx = Fixture::heavy_repo(10);
    fs::write(fx.repo.join(".flashwtinclude"), "heavy/\n").unwrap();
    let out = fx.flashwt_env(
        &["create", "ladder-forced"],
        &[
            ("FLASHWT_SNAPSHOTS", "0"),
            ("FLASHFLASHWT_SNAPSHOTS_V2", "0"),
        ],
    );
    ...
    assert!(
        !stdout.contains("via snapshot"),
        "explicit FLASHWT_SNAPSHOTS=0 must opt out: {stdout}"
    );
}
...
fn lockfile_parser_safety_classification() {
    use flashwt_store::lockfile::{DependencySafety, classify_lockfile};
...
#[cfg(target_os = "macos")]
const SNAPSHOTS_ON: &[(&str, &str)] = &[("FLASHWT_SNAPSHOTS", "1")];
#[cfg(target_os = "macos")]
const SNAPSHOTS_OFF: &[(&str, &str)] = &[
    ("FLASHWT_SNAPSHOTS", "0"),
    ("FLASHFLASHWT_SNAPSHOTS_V2", "0"),
];
...
fn timing_lines_attribute_snapshot_lookup_build_and_clone() {
    ...
    let env: &[(&str, &str)] = &[("FLASHWT_SNAPSHOTS", "1"), ("FLASHWT_TIMING", "1")];
    ...
    for line in [
        "flashwt-stage snapshot=",
        "flashwt-stage snapshot-lookup=",
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("flashwt-stage snapshot-lookup="),
        "warm run must report snapshot-lookup:\n{stderr}"
    );
    assert!(
        stderr.contains("flashwt-stage snapshot-clonefile="),
...
fn miss_builds_publishes_and_second_create_hits_with_private_files() {
    ...
    let published: Vec<String> = fs::read_dir(&snapshots_dir)
        ...
        .filter(|n| flashwt_store::ContentId::from_hex(n).is_some())
        .collect();
    assert_eq!(published.len(), 1, "exactly one snapshot expected");

    let mirrors = flashwt_store::read_mirrors(&fx.store_path());
    ...
    let out = fx.flashwt_env(
        ...
        &[("FLASHWT_SNAPSHOTS", "1"), ("FLASHWT_TIMING", "1")],
    );
    assert_created(&out);
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("flashwt-stage snapshot="),
...
#[cfg(target_os = "macos")]
#[test]
fn flashwt_verify_bypasses_hits_and_hashes_every_blob() {
    let fx = Fixture::rich_repo();

    assert_created(&fx.flashwt_env(
        &[
            "create",
            "one",
            "--dir",
    ...
    let out = fx.flashwt_env(
        ...
        &[("FLASHWT_SNAPSHOTS", "1"), ("FLASHWT_VERIFY", "1")],
    );
    assert!(
        !out.status.success(),
...
fn incremental_rebuild_guard_wide_diff_surfaces_fallback_in_json_and_timing() {
    ...
    let stderr = String::from_utf8_lossy(&out_timing.stderr);
    assert!(
        stderr.contains("flashwt-stage snapshot-mode=build"),
...
fn evicted_snapshot_referenced_by_mirror_rebuilds_on_next_create() {
    ...
    let hash = fs::read_dir(&snapshots_dir)
        .unwrap()
        .map(|e| e.unwrap().file_name().to_string_lossy().into_owned())
        .find(|n| flashwt_store::ContentId::from_hex(n).is_some())
    ...
    let store = flashwt_store::DiskStore::open(fx.store_path()).unwrap();
    let report = store
...
fn gc_marks_through_valid_manifests_only() {
    ...
    let mut store = flashwt_store::DiskStore::open(base.path().join("store")).unwrap();
    ...
    assert_eq!(
        store
            ...
            .unwrap()
            .outcome,
        flashwt_store::PublishOutcome::Published
    );

    let worktree_dir = base.path().join("worktree");
    fs::create_dir_all(&worktree_dir).unwrap();
    let gitdir = base.path().join("worktree.git");
...
#[cfg(target_os = "macos")]
const BOTH_GATES: &[(&str, &str)] = &[
    ("FLASHWT_SNAPSHOTS", "1"),
    ("FLASHFLASHWT_SNAPSHOTS_V2", "1"),
];
...
fn bump_rebuilds_incrementally_and_matches_source_exactly() {
    ...
    let linked = stderr
        .lines()
        .find_map(|l| l.strip_prefix("flashwt-stage snapshot-v2-linked="))
        .map(|v| v.trim().parse::<usize>().expect("integer linked count"));
    let linked = linked.expect("snapshot-v2-linked line must be present");

    let cloned = stderr
        .lines()
        .find_map(|l| l.strip_prefix("flashwt-stage snapshot-v2-cloned="))
...
fn paranoid_verify_with_v2_gates_still_lands_an_exact_tree() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("flashwt-stage snapshot-mode="),
...
#[cfg(target_os = "macos")]
fn manifest_of(store: &Path, hash: &str) -> flashwt_store::Manifest {
    let id = flashwt_store::ContentId::from_hex(hash).expect("hex hash");
    let text =
        fs::read_to_string(flashwt_store::snapshot_path(store, &id).join("manifest.tsv")).unwrap();
    flashwt_store::Manifest::parse(&text).expect("valid manifest")
}

#[cfg(target_os = "macos")]
fn blob_ids(manifest: &flashwt_store::Manifest) -> BTreeSet<flashwt_store::ContentId> {
    manifest.entries.iter().filter_map(|e| e.blob).collect()
}

#[cfg(target_os = "macos")]
fn object_path(store: &Path, id: &flashwt_store::ContentId) -> PathBuf {
...
fn assert_no_incomplete_published(store: &Path, iteration: usize) {
    for name in published_hashes(store) {
        let hash = flashwt_store::ContentId::from_hex(&name).expect("hex address");
        assert!(
            flashwt_store::read_published_snapshot(store, &hash).is_some(),
...
fn sigkilled_creates_never_leave_a_half_published_snapshot_behind() {
    ...
    for (i, delay_ms) in KILL_DELAYS_MS.iter().enumerate() {
        let mut child = Command::new(env!("CARGO_BIN_EXE_flashwt"))
            ...
            .env("FLASHWT_STORE", fx.store_path())
            .envs(BOTH_GATES.iter().copied())
            .env_remove("FLASHWT_HARDLINK")
            .env_remove("FLASHWT_NO_HARDLINK")
            .env_remove("FLASHWT_GC_GRACE")
            .current_dir(&fx.repo)
            .spawn()
            .expect("spawn flashwt create");
...
fn sweep_evicts_unreferenced_old_generation_but_shared_blobs_survive() {
    ...
    assert!(
        !flashwt_store::snapshot_path(
            &store,
            &flashwt_store::ContentId::from_hex(&a_hash).unwrap()
        )
        .exists()
    );
    assert!(
        flashwt_store::snapshot_path(&store, &flashwt_store::ContentId::from_hex(b_hash).unwrap())
            .exists()
    );
...
fn sweep_keeps_old_generation_alive_when_newer_snapshot_dir_vanishes() {
    ...
    fs::remove_dir_all(flashwt_store::snapshot_path(
        &store,
        &flashwt_store::ContentId::from_hex(&b_hash).unwrap(),
    ))
    .unwrap();
...
fn incremental_snapshot_tree_files_are_private_and_self_contained() {
    ...
    let sample = flashwt_store::snapshot_tree_path(
        &store,
        &flashwt_store::ContentId::from_hex(&b_hash).unwrap(),
    )
