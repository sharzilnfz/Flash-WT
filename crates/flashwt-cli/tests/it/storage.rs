...
use flashwt_copy::{BackendKind, SourcePolicy, candidates, select_backend};
use flashwt_store::{DiskStore, FsCapabilities, probe_fs};
...
fn flashwt_cow_default(fx: &Fixture, args: &[&str], store: &Path) -> std::process::Output {
...
fn default_hydration_produces_private_writable_files() {
    ...
    for name in ["one", "two"] {
        let out = flashwt_cow_default(&fx, &["create", name], &store);
...
fn in_place_rewrite_succeeds_and_stays_private_to_its_worktree() {
    ...
    let out = flashwt_cow_default(&fx, &["create", "three"], &store);
    assert_created_ok(&out);
        ...
        impl HfsRamdisk {
            fn attach() -> Option<HfsRamdisk> {
                ...
                let formatted = Command::new("diskutil")
                    .args(["eraseVolume", "HFS+", "flashwt-cow-test"])
                    .arg(&device)
                    .output()
                    .ok()?;
                ...
                let mount = PathBuf::from("/Volumes/flashwt-cow-test");
                if !mount.is_dir() {
                    detach(Some("flashwt-cow-test"));
                    return None;
                }
...
fn flashwt_hardlinked(fx: &Fixture, name: &str, store: &Path) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_flashwt"))
        .args(["create", name])
        .env("FLASHWT_STORE", store)
        .env("FLASHWT_HARDLINK", "1")
        .env("FLASHWT_SNAPSHOTS", "0")
        .env("FLASHFLASHWT_SNAPSHOTS_V2", "0")
        .current_dir(&fx.repo)
        .output()
        .expect("run flashwt binary")
}
...
fn opting_into_hardlinks_shares_one_inode_per_blob_across_worktrees() {
    ...
    let store = base.path().join("store");
    let parent = fx.repo.parent().unwrap();
    let flashwt_fn = |name: &str| flashwt_hardlinked(&fx, name, &store);

    let first = flashwt_fn("one");
    assert!(
...
fn opted_in_hardlinks_refuse_in_place_rewrites_and_protect_siblings() {
    ...
    let parent = fx.repo.parent().unwrap();
    let flashwt_fn = |name: &str| flashwt_hardlinked(&fx, name, &store);
    assert!(flashwt_fn("one").status.success(), "create one failed");
    assert!(flashwt_fn("two").status.success(), "create two failed");
    ...
    assert!(flashwt_fn("three").status.success(), "create three failed");
    let three = file_snapshot(&parent.join("origin-three").join("heavy"));
...
fn second_worktree_adds_no_new_store_content() {
    ...
    let store = base.path().join("store");
    let flashwt_fn = |name: &str| fx.flashwt_with_store(&["create", name], &store);
    ...
    for name in ["one", "two"] {
        ...
        let ledger = Path::new(git_dir).join("flashwt-hydrated.tsv");
...
fn hash_mismatch_during_materialize_fails_loudly() {
    ...
    let non_snap: &[(&str, &str)] = &[
        ("FLASHWT_SNAPSHOTS", "0"),
        ("FLASHFLASHWT_SNAPSHOTS_V2", "0"),
    ];
    let first = fx.flashwt_with_store_env(&["create", "good"], &store, non_snap);
    ...
    let second = fx.flashwt_with_store_env(&["create", "bad"], &store, non_snap);
    assert!(
        !second.status.success(),
...
fn hydrated_trees_preserve_symlinks_and_permission_bits() {
    ...
    let store = base.path().join("store");
    for name in ["one", "two"] {
        let out = fx.flashwt_with_store(&["create", name], &store);
        assert!(
            out.status.success(),
            "create {name} failed: {}",
            String::from_utf8_lossy(&out.stderr)
...
fn hardlink_mode_preserves_exec_bits_without_touching_the_shared_blob() {
    ...
    let base = tempfile::tempdir().expect("tempdir");
    let store = base.path().join("store");
    let out = fx.flashwt_with_store_env(
        &["create", "one"],
        &store,
        &[("FLASHWT_HARDLINK", "1"), ("FLASHWT_SNAPSHOTS", "0")],
    );
    assert!(
        out.status.success(),
...
fn create_with_base_records_symbolic_ref_and_initial_commit_in_mirror() {
    ...
    let out = fx.flashwt_with_store(&["create", "stack-1", "--base", "main"], store_dir.path());
    assert!(out.status.success());

    let mirrors = flashwt_store::mirror::read_all(store_dir.path());
...
fn base_movement_detected_in_json_and_human_output() {
    ...
    let out1 = fx.flashwt(&["create", "feat-1", "--base", "main"]);
    ...
    let out_stacked = fx.flashwt(&["create", "feat-stacked", "--base", "feat-1", "--json"]);
...
fn no_diagnostic_when_base_has_not_moved() {
    ...
    let out = fx.flashwt(&["create", "feat-clean", "--base", "main", "--json"]);
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
    let json: serde_json::Value = serde_json::from_str(stdout.trim()).expect("parse json");
    let diags = json["diagnostics"].as_array().expect("diagnostics array");
...
fn remove_surfaces_base_movement_diagnostic() {
    ...
    let out_create = fx.flashwt(&["create", "feat-to-remove", "--base", "main"]);
    ...
    let out_remove = fx.flashwt(&["remove", "feat-to-remove", "--json"]);
...
"#,
    ...
    let flashwt_out = Command::new(env!("CARGO_BIN_EXE_flashwt"))
        .args(["create", "demo"])
        .env("FLASHWT_STORE", &store_dir)
        .current_dir(&repo)
        .output()
        .unwrap();

    assert!(flashwt_out.status.success());
...
fn test_starter_manifest_and_volatile_cache_exclusions() {
    ...
    let starter = fs::read_to_string(fx.repo.join(".flashwtinclude")).unwrap();
...
fn copy_file_range_accelerated_materialization_produces_identical_trees() {
    ...
    let out = fx.flashwt(&["create", "accel", "--json"]);
    assert!(out.status.success());
...
fn sequential_buffered_copy_preserves_executable_mode_and_content() {
    ...
    let copied_bytes = flashwt_copy::buffered_copy_file(&src, &dest).expect("buffered_copy_file");
...
fn flashwt_verify_run(
    ...
    cmd.output().expect("run flashwt binary")
}
...
fn second_create_succeeds_on_tampering_no_stat_can_see() {
    ...
    let first = flashwt_verify_run(&fx, &["create", "one"], &store, &[]);
    assert!(first.status.success());
    assert!(store.join("verified.tsv").is_file());

    tamper_verify_blob(&store, true);

    let second = flashwt_verify_run(&fx, &["create", "two"], &store, &[]);
...
fn tampering_that_moves_mtime_still_fails_loudly() {
    ...
    let first = flashwt_verify_run(&fx, &["create", "good"], &store, &[]);
    assert!(first.status.success());

    tamper_verify_blob(&store, false);

    let second = flashwt_verify_run(&fx, &["create", "bad"], &store, &[]);
    assert!(!second.status.success());
    let stderr = String::from_utf8_lossy(&second.stderr);
...
#[test]
fn flashwt_verify_catches_even_stat_invisible_tampering() {
    let (fx, store) = verify_fixture(20);
    ...
    let paranoid = flashwt_verify_run(&fx, &["create", "two"], &store, &[("FLASHWT_VERIFY", "1")]);
