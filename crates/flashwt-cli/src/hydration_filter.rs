...
# flashwt: directories hydrated into every new worktree.
...
pub fn load_patterns(root: &Path, manifest: Option<&Path>) -> Result<LoadedPatterns> {
    if let Some(m) = manifest {
        ...
        let default_path = root.join(".flashwtinclude");
...
mod tests {
    ...
    fn missing_explicit_manifest_is_an_error_and_default_uses_in_memory_defaults() {
        ...
        let err = load_patterns(root, Some(&root.join("nope.flashwtinclude"))).unwrap_err();
        ...
        assert!(!root.join(".flashwtinclude").exists());

        write_starter_manifest(&root.join(".flashwtinclude")).unwrap();
        assert!(root.join(".flashwtinclude").is_file());

        let loaded = load_patterns(root, None).unwrap();
        match loaded {
            LoadedPatterns::Loaded { patterns, path } => {
                assert_eq!(path, root.join(".flashwtinclude"));
