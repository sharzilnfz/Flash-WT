...
worktree /path/to/bare-worktree
bare
";
        ...
        assert_eq!(parsed[2].path, PathBuf::from("/path/to/detached-worktree"));
        assert!(parsed[2].is_detached);
        assert_eq!(parsed[2].branch, None);

        assert_eq!(parsed[3].path, PathBuf::from("/path/to/bare-worktree"));
    ...
    fn test_rev_parse_cache_git_dir_and_resolve_git_dir() {
        ...
        let gd1 = engine.resolve_git_dir(&worktree_path);
        assert!(gd1.exists());
        assert_eq!(engine.cache_hits(), 0);
        assert_eq!(engine.cache_misses(), 1);

        let gd2 = engine.git_dir(&worktree_path).unwrap();
        assert_eq!(gd1, gd2);
        assert_eq!(engine.cache_hits(), 1);
        assert_eq!(engine.cache_misses(), 1);

        let gd3 = engine.git_dir(&worktree_path).unwrap();
        assert_eq!(gd1, gd3);
        assert_eq!(engine.cache_hits(), 2);
        assert_eq!(engine.cache_misses(), 1);
    }
    ...
    fn test_rev_parse_cache_invalidation_on_removal() {
        ...
        assert!(gd.exists());
        assert!(engine.cache_has_git_dir(&worktree_path));

        engine.remove_worktree_force(&worktree_path).unwrap();
        assert!(!engine.cache_has_git_dir(&worktree_path));
