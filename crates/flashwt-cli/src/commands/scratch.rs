...
pub fn run(
    ...
    let pid = if let Some(owner) = std::env::var("FLASHWT_OWNER_PID")
        .ok()
    ...
    let start_time =
        flashwt_store::process_start_time(pid).unwrap_or_else(current_process_start_time);
    ...
    let lease_file_path = publish_lease(&store_root, &lease).map_err(|e| {
        Error::io(
            "write lease file",
            flashwt_store::lease_path(&store_root, &lease_id),
            e,
        )
    })?;
    ...
    match run_cmd {
        ...
        Some(cmd) => {
            if !cfg.json {
                eprintln!("flashwt-scratch: running `{cmd}` in {}", dest.display());
            }
