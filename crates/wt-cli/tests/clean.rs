...
fn wt_clean_unregisters_raw_git_worktree_and_spares_sibling() {
    ...
    assert!(out.status.success(), "wt clean failed: {}", String::from_utf8_lossy(&out.stderr));
    ...
    assert!(!registered.contains("origin-raw-a"), "git must forget the removed worktree: {registered}");
    assert!(registered.contains("origin-raw-b"), "untouched sibling must stay registered: {registered}");
