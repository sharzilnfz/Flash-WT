...
//! Integration tests verifying that legacy sweepers never collect member blobs
//! referenced by active snapshot trees during the dual-write transition.
...
fn legacy_sweep_does_not_collect_active_pinned_snapshot_member_blobs() {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        assert_eq!(
            fs::read(wt_dir.path().join("node_modules/pkg/index.js")).expect("read hydrated file"),
            content
        );

        let mut reclaimer = StoreReclaimer::new(&mut store);
        let receipt = reclaimer
            .retire_worktree(wt_dir.path(), git_dir.path())
